...
class ChangeSetFileDb(fsdb.GenericChangeFileDb):
    FileDir = TopPatchFileDb.FileDir
    ...
    def is_current(self):
        return True
    def _get_patch_data_text(self, h):
        return runext.run_cmd(["hg", "status", "-mardC", "--change", str(self._rev)]).stdout
    def _iterate_file_data(self, pdt):
        return iterate_hg_file_data(pdt, "")

class IncomingChangeSetFileDb(fsdb.GenericChangeFileDb):
    FileDir = TopPatchFileDb.FileDir
    ...
    def is_current(self):
        return True
    def _get_patch_data_text(self, h):
        cmd = ["hg", "-q", "incoming", "--template", "{files}\\n{file_adds}\\n{file_dels}\\n", "-nl", "1", "--rev", str(self._rev)]
        if self._path:
    ...
    def _iterate_file_data(self, pdt):
        ...
        for file_path in utils.string_to_file_list(lines[0]):
            if file_path in added_files:
                yield (file_path, FSTATUS_ADDED, None)
            elif file_path in deleted_files:
                yield (file_path, FSTATUS_REMOVED, None)
            else:
                yield (file_path, FSTATUS_MODIFIED, None)
        return

class CommitFileDb(fsdb.GenericChangeFileDb):
    FileDir = TopPatchFileDb.FileDir
    def __init__(self, file_paths_filter=None):
        self._file_paths_filter = file_paths_filter if file_paths_filter else []
        fsdb.GenericChangeFileDb.__init__(self)
    @property
    def is_current(self):
        h = hashlib.sha1()
        ...
        return h.digest() == self._db_hash_digest
    def _get_patch_data_text(self, h):
        patch_status_text = runext.run_cmd(["hg", "status", "-mardC"] + self._file_paths_filter).stdout
        h.update(patch_status_text)
        resolve_list_text = runext.run_cmd(["hg", "resolve", "--list"] + self._file_paths_filter).stdout
        h.update(resolve_list_text)
        return (patch_status_text, resolve_list_text)
    def _iterate_file_data(self, pdt):
        return iterate_hg_file_data(*pdt)
